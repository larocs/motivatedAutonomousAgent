...
/**
 * @file sleepingModule.cpp
 * @brief Implementation of the sleepingModule (see header file).
 */
...
bool sleepingModule::configure(yarp::os::ResourceFinder &rf) {
    ...
    */
    ...
    //pThread = new sleepingThread(robotName, rf);
    pThread = new sleepingThread(robotName, rf, robotPlatform);
...
bool sleepingModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}

bool sleepingModule::close() {
    handlerPort.close();
    /* stop the thread */
...
bool sleepingModule::respond(const Bottle& command, Bottle& reply) 
{
...
bool sleepingModule::updateModule()
{
    return true;
}

double sleepingModule::getPeriod()
{
