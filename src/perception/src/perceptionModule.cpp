 ...
 * @brief Implementation of the perceptionModule (see header file).
 */
...
bool perceptionModule::configure(yarp::os::ResourceFinder &rf) {
    ...
    */
    ...
    //pThread = new perceptionThread(robotName, configFile);
    pThread = new perceptionThread(robotName, rf);
...
bool perceptionModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}

bool perceptionModule::close() {
    handlerPort.close();
    /* stop the thread */
...
bool perceptionModule::respond(const Bottle& command, Bottle& reply) 
{
...
bool perceptionModule::updateModule()
{
    return true;
}

double perceptionModule::getPeriod()
{
